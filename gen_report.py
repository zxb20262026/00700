#!/usr/bin/env python3
"""腾讯控股 日监控 — HTML报告生成器 v1.2

Tencent daily monitor HTML generator
"""
...
def build_header(data):
  ...
  <a class="nav-btn active" href="https://zxb20262026.github.io/00700/">🐧 腾讯控股</a>
...
<div class="header">
  <h1>🐧 腾讯控股 · 日监控</h1>
  <div class="sub">腾讯控股日监控 · Tencent Daily Monitor · {data["date"]}</div>
  <div class="sub" style="margin-top:2px">港股 <span style="color:{color_pct(chg)};font-weight:700">HK${price} {chg_str}</span></div>
  <span class="badge {bc}">{mode}</span>
</div>'''


def build_kpi_dashboard(data):
    """8张KPI卡片：腾讯/PE/PEG/ROE/市值/恒生科技/MA60/52周位置"""
    a = data.get("tencent", {})
    full = data.get("tencent_full", {})
    pe = data.get("tencent_pe", {})
    peg = data.get("peg")
    ...
    cards.append(make_card("腾讯控股", f"HK${price:.2f}" if price else "—",
                           f"{icon_pct(chg)} {sign(chg)}{chg:.1f}%" if chg is not None else "",
                           note_a, color_pct(chg)))

    # 2. PE(TTM)
    pe_val = pe.get("pe_ttm") if pe else full.get("pe_ttm")
    pe_note = s.get("pe", {}).get("text", "")
    pe_clr = "#3fb950" if pe_val and pe_val < 18 else "#d29922" if pe_val and pe_val < 25 else "#f85149"
    ...
    # 4. ROE
    roe_val = full.get("roe")
    roe_clr = "#3fb950" if roe_val and roe_val > 15 else "#d29922" if roe_val and roe_val > 8 else "#f85149"
    roe_note = f"ROE={roe_val:.1f}%" if roe_val else "—"
    roe_note += "\n优秀>15% 良好>10%" if not roe_val or roe_val < 15 else "\n盈利能力优秀"
    cards.append(make_card("ROE", f"{roe_val:.1f}%" if roe_val else "—", "", roe_note, roe_clr))

    # 5. 总市值
    mc = full.get("market_cap")
    mc_str = f"{mc:,.0f}亿" if mc else "—"
    pb = full.get("pb")
    mc_note = f"≈ {mc/10000:.2f}万亿港元" if mc and mc > 10000 else ""
    if pb: mc_note += f" · PB {pb:.1f}x"
    cards.append(make_card("总市值", mc_str, "", mc_note, "#58a6ff"))

    # 6. 恒生科技
    ind = s.get("industry", {})
    ind_chg = ind.get("change_pct")
    ind_name = ind.get("name", "恒生科技")
    vs_ind = s.get("vs_industry", "")
    ind_note = f"vs指数: {vs_ind}" if vs_ind else ""
    cards.append(make_card(f"{ind_name}", f"{sign(ind_chg)}{ind_chg:.1f}%" if ind_chg is not None else "—",
                           "", ind_note, color_pct(ind_chg)))

    # 7. 52周位置
    high52 = full.get("high_52w")
    low52 = full.get("low_52w")
    if high52 and low52 and price:
        pos = round((price - low52) / (high52 - low52) * 100)
        pos_note = f"距高点 {sign(price-high52)}{round((price-high52)/high52*100,1)}%"
        pos_note += f"\n52周: HK${low52:.0f}-{high52:.0f}"
        pos_clr = "#3fb950" if pos < 30 else "#d29922" if pos < 70 else "#f85149"
        cards.append(make_card("52周位置", f"{pos}%分位", "", pos_note, pos_clr))
    else:
        cards.append(make_card("52周位置", "—", "", "数据暂缺", "#8b949e"))
    ...
    if ma60_val:
        ma60_note = f"MA60=HK${ma60_val:.0f} 距{sign(ma60_dist)}{abs(ma60_dist):.2f}%"
        if ma60_near: ma60_note += "\n临近核心支撑"
    else:
        ma60_note = "—"
    cards.append(make_card("MA60", f"HK${ma60_val:.0f}" if ma60_val else "—", "", ma60_note, ma60_clr))
...
def build_summary_panel(data):
    ...
    if peg_v and peg_v > PEG_OVERVALUE:
        tp_display = f"HK${tp:.0f}" if tp else "—"
    ...
    if ma60.get("value") and mas:
        ...
        if price and ma60v:
            ...
            ma_parts = []
            if ma5 and price < ma5: ma_parts.append(f"跌破MA5(HK${ma5:.0f})")
            if ma20 and price < ma20: ma_parts.append(f"MA20(HK${ma20:.0f})")
            if near:
                ma_parts.append(f'<span class="hl-yellow">逼近MA60(HK${ma60v:.0f})核心支撑</span>')
                ma_parts.append("若有效跌破则技术面转弱")
            else:
                ma_parts.append(f"MA60=HK${ma60v:.0f} (距{sign(dist)}{abs(dist):.2f}%)")
    ...
    if li_text and "暂缺" not in li_text:
        ...
        items.append({"level": level, "icon": "🏢", "html": li_text + " → 成本端影响"})
...
def build_trading_plan(data):
    ...
    if mas:
        lines = []
        if mas.get("MA5"): lines.append(f'MA5=HK${mas["MA5"]:.0f} (短期压力)')
        if mas.get("MA20"): lines.append(f'MA20=HK${mas["MA20"]:.0f}')
        if ma60v: lines.append(f'<span class="hl-yellow">MA60=HK${ma60v:.0f} (核心支撑)</span>')
        if lines:
            sr_html = f'<p>{" · ".join(lines)}</p><p>心理关口: HK$400</p>'

    # ── 加仓触发条件 ──
    trigger_html = ""
    if ma60v:
        trigger_html = f'''<ul>
<li>条件A: MA60(HK${ma60v:.0f})附近企稳(连续2日收盘高于MA60) → 第一批加仓</li>
...
<li>条件C: PEG进一步下降至0.6以下 → 一次性加仓</li>
    ...
    if ma60v:
        plan_html = f'''<ul>
<li>第一批: HK${ma60v-5:.0f}-{ma60v+3:.0f} × 30% (MA60支撑区)</li>
<li>第二批: HK$393-400 × 40% (心理关口+恐慌区)</li>
<li>第三批: HK$385以下 × 30% (极端情况，需重新评估基本面)</li>
</ul>'''
    else:
    ...
    if eps and tp_data.get("mid") and tp_data["mid"] > 0:
        target_html = f'<p>买入价(PEG=1): HK${tp_data["buy"]:.0f} · 目标区间: HK${tp_data["low"]:.0f}-{tp_data["high"]:.0f} (基于PE {TRADING["target_pe_range"][0]}-{TRADING["target_pe_range"][1]}x)</p>'
    else:
        target_html = '<p>待采集PE数据后自动计算</p>'

    stop_loss = TRADING.get("stop_loss_price")
    stop_html = f'<p>止损位: HK${stop_loss} (有效跌破视为逻辑破坏)</p>' if stop_loss else '<p>止损: 以PE>30x为参考</p>'

    # ── 风险收益比 ──
    if price and eps and tp_data.get("mid") and tp_data["mid"] > 0:
        potential_up = round((tp_data["mid"] - price) / price * 100, 1)
        potential_down = round((price - stop_loss) / price * 100, 1) if stop_loss else 0
...
def build_week_review(data):
    ...
    for d in days:
        ...
        rows += f'<tr><td>{d["date"][-5:]}</td><td>HK${d["close"]:.2f}</td><td style="color:{clr}">{d["change_pct"]:+.1f}%</td><td style="color:{fn_clr}">{fn_str}</td><td style="font-size:0.72em;color:#8b949e">{d["events"]}</td></tr>'
...
def build_valuation(data):
    ...
    for i, (label, color) in enumerate(band_defs):
        b = bands.get(label, {})
        bp = b.get("price", 0)
        band_rows += f'<tr><td style="color:{color}">{label}</td><td>{b.get("pe","")}x</td><td>{eps_val or "—"}</td><td style="color:{color};font-weight:600">HK${bp:.0f}</td></tr>'
        ...
        # 价格标签 — 标在区间右边界（分界线上）
        price_labels += f'<span style="position:absolute;left:{(i+1)*seg_width}%;transform:translateX(-50%);font-size:0.58em;color:#8b949e;top:-16px;z-index:1">HK${bp:.0f}</span>'
    ...
    # 当前位置说明
    if current_band == "保守区":
        cur_note = f'当前HK${price:.0f} 处于<span class="hl-yellow">保守区</span>，PE={pe_ttm:.1f}x，距合理区下沿HK${bands.get("合理区",{}).get("price",0):.0f}仅差{bands.get("合理区",{}).get("price",0)-price:.0f}元'
    elif current_band == "合理区":
        cur_note = f'当前HK${price:.0f} 处于<span class="hl-yellow">合理区</span>，PE={pe_ttm:.1f}x'
    else:
        cur_note = f'当前HK${price:.0f} 处于{current_band}，PE={pe_ttm:.1f}x'

    # ── 子板块3: 同行估值对比 ──
    peer_rows = ""
    for name, pv in val.get("peers", {}).items():
        highlight = 'style="font-weight:700;color:#58a6ff"' if name == "腾讯控股" else ""
  ...
  <div class="val-section">
    ...
    <div class="inst-cards">
      ...
      <div class="inst-card"><div class="lbl">机构目标均价</div><div class="val" style="color:#d29922">HK${target_avg:.0f}</div><div class="note">{target_note}</div></div>
    </div>
  </div>
</div>'''
...
def build_technical_analysis(data):
    ...
    def _dev_card(label, val, color, price_ref):
        ...
        return f'''<div class="tech-card">
          ...
          <div class="t-val" style="color:{color}">HK${val:.2f}</div>
...
def _build_tech_chart(kline, mas):
    ...
    for i in range(5):
        v = y_min + i * y_range / 4
        y = PAD + (1 - (v - y_min) / y_range) * plot_h
        y_ticks += f'<text x="{PAD - 6}" y="{y + 3:.1f}" text-anchor="end" font-size="9" fill="#484f58">HK${v:.0f}</text>'
...
def _build_tech_summary(data, mas, price, chg_pct, chg_str, streak):
    ...
    lines.append(f'<span class="ts-key">📌 今日走势</span>：腾讯控股<span class="ts-key">{chg_str}</span>'
    ...
    if above_ma60 and dev60 and abs(dev60) < 3:
        lines.append(f'<span class="ts-key">💼 操作参考</span>：股价在MA60支撑位附近，是<b>观察窗口而非操作窗口</b>。'
                     f'长线投资者可考虑在MA60附近分批金字塔式建仓（例如HK${ma60:.0f}上方分3-5批），'
        ...
        lines.append(f'<span class="ts-key">💼 操作参考</span>：股价在MA60上方{dev60:.1f}%，安全边际充足。'
                     f'长线持有者可继续持有，若回踩MA60(HK${ma60:.0f})不破则是加仓良机。')
    elif not above_ma60:
...
def build_ah_analysis(data):
    """腾讯是纯港股，无AH溢价。此模块展示南向资金/港股通概况"""
    return '''<div class="module">
  <div class="module-hdr"><span class="icon">🐧</span><h2>港股市场定位</h2></div>
  <div style="padding:20px;text-align:center;color:#6e7681;line-height:1.8">
    <p style="font-size:1.1em;color:#58a6ff">腾讯控股 (00700.HK) 仅在港交所上市</p>
    <p>无A/H股溢价问题，估值直接对标港股科技板块</p>
    <p style="margin-top:8px;font-size:0.85em">参考恒生科技指数 · 恒生指数</p>
  </div>
</div>'''
...
def _build_ah_chart(ah_history, ah_mean30, ah_min, ah_max):
    ...
    # 图例
    legend = f'<line x1="{PAD + 8}" y1="10" x2="{PAD + 24}" y2="10" stroke="#d29922" stroke-width="2"/><text x="{PAD + 28}" y="14" font-size="9" fill="#8b949e">南向资金率</text>'
    if ah_mean30:
...
def _build_ah_deep(data, ah_disabled, ah_mean30, ah_history, ranking, a_price, h_price, h_cny, dev_30):
    """生成南向资金深度解读"""
    a = data.get("tencent", {})
    h = data.get("_tencent_h_removed", {})
    lines = []

    if ah_disabled is None:
        lines.append('数据暂缺，无法生成解读。')
        return "".join(f"<li>{l}</li>" for l in lines)

    # 1. 极端折价判断
    if ah_disabled < -25:
        lines.append(f'<span class="ah-red">极端折价创历史新高</span>：<span class="ah-gold">{ah_disabled:.2f}%</span>意味着同样的腾讯控股股权，A股比港股便宜约<span class="ah-gold">{abs(ah_disabled):.0f}%</span>。在AH股全市场中<span class="ah-red">折价排名第{ranking["rank"]}位</span>（共{ranking["total"]}只），处于历史极端水平。')
    elif ah_disabled < -10:
        lines.append(f'<span class="ah-gold">持续折价</span>：南向资金率<span class="ah-gold">{ah_disabled:.2f}%</span>，A股较港股折价约{abs(ah_disabled):.0f}%，在AH全市场排名第{ranking["rank"]}/{ranking["total"]}。')
    elif ah_disabled > 0:
        lines.append(f'A股<span class="ah-blue">溢价港股</span>{ah_disabled:.2f}%，AH全市场排名第{ranking["rank"]}/{ranking["total"]}。')
    else:
        lines.append(f'南向资金率{ah_disabled:.2f}%，基本平价，AH全市场排名第{ranking["rank"]}/{ranking["total"]}。')
    ...
    if ah_disabled < -10:
        ...
        lines.append(f'当前南向资金处于正常区间，两地定价趋于一致。')

    # 3. 套利与安全边际
    if ah_disabled < -20:
        lines.append(f'<span class="ah-gold">套利逻辑</span>：极端折价通常吸引两类资金入场——(a)跨市场套利资金买入A股;(b)南下资金买入H股。两者都会推动溢价向均值回归。历史数据显示，南向资金率在偏离30日均值{abs(dev_30):.1f}个百分点后，回归均值的概率超过80%。')
        lines.append(f'<span class="ah-green">安全边际</span>：对A股持有人而言，折价{abs(ah_disabled):.0f}%意味着A股具有极高的安全边际。即使H股下跌10%，A股仍有约{abs(ah_disabled)-10:.0f}%的缓冲空间。当前A股HK${a_price:.2f}，H股折人民币HK${h_cny:.2f}，A股比H股便宜HK${h_cny - a_price:.2f}。')
    elif ah_disabled < -10:
        lines.append(f'折价约{abs(ah_disabled):.0f}%，提供了一定的安全边际。A股相比H股的折价降低了A股持有者的相对成本。')

    # 4. 结论
    if ah_disabled < -25:
        lines.append(f'<span class="ah-green">结论：AH极端折价是买入A股的强化信号，不是利空</span>。在基本面未出现恶化的前提下，A股比H股便宜{abs(ah_disabled):.0f}%并非A股有问题，而是市场结构导致的定价偏差。长线投资者应将此视为安全边际的额外加成——同样的股权，A股价格打了6折。历史规律表明，极端折价终将回归，均值回归过程本身就是超额收益的来源。')
    elif ah_disabled < -10:
        lines.append(f'<span class="ah-green">结论：合理折价区间，A股具有一定性价比</span>。南向资金{ah_disabled:.2f}%处于折价区间，A股对长线持有者仍有吸引力，但安全边际不如极端折价时丰厚。')
    else:
        lines.append(f'当前南向资金在合理范围内，不影响A股的核心投资逻辑。')
...
def build_peg_analysis(data):
    ...
    a = data.get("tencent", {})
    pe_info = data.get("tencent_pe", {})
    val = data.get("valuation", {})
    peg_val = data.get("peg")
    catl_peer = val.get("peers", {}).get("腾讯控股", {})
    ...
    peer_pes = [p.get("pe") for p in val.get("peers", {}).values()
                if p.get("pe") and p["pe"] > 0 and p.get("name") != "腾讯控股"]
    peer_pes.sort()
    ...
    if target_avg:
        rows += _row("机构目标均价", f"HK${target_avg:.0f}",
    ...
    if eps_2026e and range_low:
        range_html = f'<div class="peg-range"><div class="pr-title">合理估值区间（基于2026E EPS HK${eps_2026e:.2f} × PE 25-30x）</div><div class="pr-value">HK${range_low:.0f} — HK${range_high:.0f}</div><div class="pr-note">较当前价有 <span style="color:#3fb950;font-weight:600">+{upside_low}% ~ +{upside_high}%</span> 上行空间</div></div>'
  ...
  <div class="peg-conclusion">
    ...
    <div class="pc-item">• 当前价位：HK${price:.2f}{chg_info}，估值安全边际充足</div>
  </div>
...
def build_analyst_consensus(data):
    ...
    if targets and price_now:
      ...
      <div class="at-title">🎯 一致预期目标区间 · 合理估值 <b style="color:#58a6ff">HK${t_low:.0f}~{t_high:.0f}</b> · 上行空间 <b style="color:{us_clr}">{upside:+.1f}%</b></div>
      <div class="at-range">
        <span class="at-label">HK${t_low:.0f}</span>
        <div class="at-bar"><div class="at-fill" style="width:100%"></div><div class="at-marker" style="left:{bar_pct}%"></div></div>
        <span class="at-label">HK${t_high:.0f}</span>
      </div>
      <div style="text-align:center;font-size:0.7em;color:#58a6ff;margin-top:4px">▲ 现价 HK${price_now:.0f} · 隐含PE {targets.get('implied_pe','—')}x</div>
    </div>'''
    ...
    if eps_data:
        ...
        for e in eps_data[:4]:
            ...
            eps_html += f'<tr><td>{e.get("year","")}</td><td style="color:{color}">{ym}</td><td class="fin-metric">{f"HK${float(eps):.2f}" if eps else "—"}</td><td>{f"{float(pe):.1f}x" if pe else "—"}</td><td>{f"{float(roe):.1f}%" if roe else "—"}</td></tr>'
        eps_html += '</table>'
    ...
    if bt.get("has_data") and bt.get("recent"):
      ...
      <div class="nf-card">
        ...
        for t in trades[:5]:
            ...
            table += f'<tr><td>{t["date"]}</td><td>HK${t["price"]:.2f}{price_gap}</td><td>{t["volume_wan"]:.0f}</td><td>{t["amount_wan"]:.0f}</td><td style="color:{buyer_clr};font-size:0.85em">{buyer_short}</td><td style="color:{seller_clr};font-size:0.85em">{seller_short}</td></tr>'
        table += '</table>'
...
def build_financial_trends(data):
    ...
    for q in quarters:
        table += f'<tr><td>{q["period"]}</td><td class="fin-metric">{q["revenue"]}</td><td class="fin-metric">{q["net_profit"]}</td><td>HK${q["eps"]}</td><td>{q["roe"]}%</td><td>{q["gross_margin"]}%</td></tr>'
    table += '</table>'
    ...
    trend_lines.append('<li>🔋 腾讯控股毛利率稳步提升至28%+，受益于—成本下降+规模效应，盈利能力持续改善</li>')
...
def build_competition_matrix(data):
    """竞争格局矩阵 — 腾讯 vs 阿里/美团/快手/网易/小米 估值对比"""
    peers = data.get("peers_valuation", {})
    tencent = data.get("tencent", {})
    full = data.get("tencent_full", {})
    
    if not peers:
        return '<div class="module"><div class="module-hdr"><span class="icon">⚔️</span><h2>竞争格局</h2></div><p style="color:#8b949e;font-size:0.8em">竞品数据采集中...</p></div>'
    
    # Build comparison table
    rows = ''
    # Tencent first
    tc_price = tencent.get("price"); tc_chg = tencent.get("change_pct")
    tc_pe = full.get("pe_ttm"); tc_pb = full.get("pb"); tc_mc = full.get("market_cap")
    tc_roe = full.get("roe")
    rows += f'<tr style="background:rgba(88,166,255,0.08);font-weight:700"><td>🐧 腾讯控股</td><td>HK${tc_price:.1f}</td><td style="color:{color_pct(tc_chg)}">{sign(tc_chg)}{tc_chg:.1f}%</td><td>{tc_pe:.1f}x</td><td>{tc_pb:.1f}x</td><td>{tc_mc:,.0f}亿</td><td>{tc_roe:.1f}%</td></tr>'
    
    for name, p in peers.items():
        if name == "腾讯控股": continue
        if p.get("error"):
            rows += f'<tr><td>{name}</td><td colspan="6" style="color:#6e7681">数据暂缺</td></tr>'
            continue
        pr = p.get("price"); ch = p.get("change_pct")
        pe_v = p.get("pe"); pb_v = p.get("pb"); mc_v = p.get("market_cap"); roe_v = p.get("roe")
        rows += f'<tr><td>{name}</td><td>HK${pr:.1f}</td><td style="color:{color_pct(ch)}">{sign(ch)}{ch:.1f}%</td><td>{pe_v:.1f}x</td><td>{pb_v:.1f}x</td><td>{mc_v:,.0f}亿</td><td>{roe_v:.1f}%</td></tr>'
    
    return f'''<div class="module">
  <div class="module-hdr"><span class="icon">⚔️</span><h2>竞争格局 · 估值对比</h2></div>
  <table><tr><th>公司</th><th>价格</th><th>涨跌</th><th>PE</th><th>PB</th><th>市值(亿)</th><th>ROE</th></tr>
  {rows}
  </table>
  <div style="margin-top:10px;font-size:0.75em;color:#8b949e;line-height:1.6">
    💡 腾讯PE 16.6x在互联网板块处于中等偏低水平 · 阿里/网易PE更低但增速也低 · 美团PE偏高(成长溢价)
  </div>
</div>'''
def build_business_ecosystem(data):
    """业务生态风向标 — 阅文/微盟/金山/众安 多周期走势"""
    mats = data.get("_materials_removed", {})
    upstream = data.get("business_proxies", {})
    avg = data.get("proxy_avg_change", 0)
    mat_cards = ""
    for name in BUSINESS_DISPLAY_ORDER:
        m = mats.get(name)
        if not m: continue
        dn = name.replace("—(电池级)", "—")
    ...
    for name, s in upstream.items():
        ...
        up_rows += f'<tr><td>{name}</td><td style="color:{clr};font-weight:600">HK${s["price"]:.2f}</td>' \
                   + pcell(s["change_pct"]) \
    ...
    return f'''<div class="module">
  <div class="module-hdr"><span class="icon">🏢</span><h2>业务生态风向标</h2><span class="status" style="background:rgba(88,166,255,0.1);color:#58a6ff">平均 {sign(avg)}{avg:.1f}%</span></div>
  <div class="mat-grid">{mat_cards}</div>
  <table style="margin-top:12px">
    <tr><th>业务线</th><th>价格</th><th>今日</th><th>5日</th><th>15日</th><th>30日</th><th>走势小结</th></tr>
    {up_rows}
  </table>
</div>'''
...
def build_competitors(data):
    ...
    rows = ""
    catl_a = data.get("tencent", {})
    cchg = catl_a.get("change_pct") if catl_a else None
    cclr = color_pct(cchg)
    rows += f'<tr style="font-weight:600"><td>腾讯控股</td><td style="color:{cclr}">HK${catl_a.get("price","—") if catl_a else "—"}</td><td style="color:{cclr}">{sign(cchg)}{cchg:.1f}%</td></tr>'
    for name, s in comps.items():
        clr = color_pct(s["change_pct"])
        rows += f'<tr><td>{name}</td><td style="color:{clr}">HK${s["price"]}</td><td style="color:{clr}">{sign(s["change_pct"])}{s["change_pct"]:.1f}%</td></tr>'
    return f'''<div class="module">
...
def build_week_news(data):
  ...
  <div style="margin-top:14px;padding-top:12px;border-top:1px solid #1e2d45">
    <div style="font-size:0.82em;font-weight:600;color:#d29922;margin-bottom:10px">📋 本周资讯对腾讯控股的影响评估</div>
    ...
    cards = [
        {
            ...
            "text": (
                f"{trend_line}"
                f"从资讯面看，{'腾讯控股本周有多条公司动态发布' if catl_titles else '本周腾讯控股公司层面暂无重大公告'}，"
        ...
        },
        {
            ...
            "text": (
                ...
                f"{'多数维持「买入」评级，目标价在HK$450-500区间，较当前价有15%-25%上行空间。' if has_analyst else '此前主流机构目标均价约HK$450，仍显著高于当前股价。'}"
                f"伴随PE持续压缩，市场给予腾讯控股的成长性溢价几乎消失，"
                f"{'此时反而是长线价值投资者收集筹码的窗口期。' if peg_sig.get('value') and peg_sig['value'] < 1 else '估值已趋于合理，向上弹性需靠业绩催化。'}"
                f"关注下周即将披露的月度排产数据，若超预期将直接催化估值修复。"
            )
        },
        {
            "icon": "🏢", "title": "成本端", "color": "#3fb950",
            "text": (
                f"上游核心原材料—最新报价{li.get('price_text','约10万元/吨')}，"
                f"{li.get('text','—价格维持在低位区间')}。"
                f"从半年维度看，—价格{'处于低位' if '偏低' in li.get('position','') else '走势平稳'}，"
                f"这对腾讯控股的电池成本构成直接利好——原材料成本约占电池总成本60%，锂价每下降10%对应毛利率提升约2个百分点。"
                f"其他关键材料方面，{'磷酸铁锂、电解钴价格同样低位运行' if '磷酸铁锂' in str(li) else '正极材料和电解液价格同步走弱'}，"
                f"整体成本环境较去年同期显著改善。"
                f"{'钠离子电池本周有重要进展。' if has_na else ''}"
                f"中长期来看，腾讯控股向上游锂矿、镍矿持续布局，原材料自供率提升将进一步熨平成本波动。"
            )
        },
        {
            ...
            "text": (
                ...
                f"腾讯控股全球动力电池装机市占率维持在37%左右，连续多年稳居第一，与第二名差距仍在拉大。"
                ...
                f"海外方面，腾讯控股匈牙利工厂、印尼项目稳步推进，全球化产能布局领先同行至少2-3年。"
            )
        },
        {
            ...
            "text": (
                ...
                f"腾讯控股作为储能电池全球出货量第一的企业，是政策红利最直接的受益方。"
        ...
        },
        {
            ...
            "text": (
                ...
                f"建议在HK$390-HK$400区间金字塔式建仓，止损位设在MA60下方3%（约HK$385）。"
                f"对于短线交易者：趋势未扭转前保持观望，等待放量阳线确认企稳信号再入场。"
                f"下周重点关注：月度排产数据、大宗交易机构动向、以及—价格是否继续下探。"
            )
        },
    ]
    html = ""
...
def build_pe_history_chart(data):
    """PE历史分位图 — SVG走势"""
    pe_history = data.get("pe_history", [])
    full = data.get("tencent_full", {})
    curr_pe = full.get("pe_ttm") or (data.get("tencent_pe", {}).get("pe_ttm"))
    
    if not pe_history or len(pe_history) < 10:
        return '<div class="module"><div class="module-hdr"><span class="icon">📈</span><h2>PE历史分位</h2></div><p style="color:#8b949e;font-size:0.8em;text-align:center;padding:20px">PE历史数据不足（需更多K线数据）</p></div>'
    
    pes = [p["pe"] for p in pe_history]
    pe_min = min(pes); pe_max = max(pes)
    if pe_max == pe_min: pe_max = pe_min + 5
    
    # Stats
    pe_mean = sum(pes) / len(pes)
    sorted_pes = sorted(pes)
    p25 = sorted_pes[len(sorted_pes)//4]
    p50 = sorted_pes[len(sorted_pes)//2]
    p75 = sorted_pes[3*len(sorted_pes)//4]
    
    percentile = round(sum(1 for p in pes if p <= curr_pe) / len(pes) * 100) if curr_pe else 0
    pct_color = "#3fb950" if percentile < 30 else "#d29922" if percentile < 70 else "#f85149"
    
    # Generate SVG
    w, h, pad_l, pad_r, pad_t, pad_b = 800, 200, 50, 20, 20, 30
    pw = w - pad_l - pad_r; ph = h - pad_t - pad_b
    
    points = []
    for i, pe_v in enumerate(pes):
        x = pad_l + (i / max(1, len(pes)-1)) * pw
        y = pad_t + (1 - (pe_v - pe_min) / (pe_max - pe_min)) * ph
        points.append(f"{x:.1f},{y:.1f}")
    
    # Mean line
    mean_y = pad_t + (1 - (pe_mean - pe_min) / (pe_max - pe_min)) * ph
    # Current PE marker
    curr_y = pad_t + (1 - (curr_pe - pe_min) / (pe_max - pe_min)) * ph if curr_pe else mean_y
    
    svg = f'''<svg viewBox="0 0 {w} {h}" style="width:100%;max-width:800px">
  <rect width="{w}" height="{h}" fill="#0d1117" rx="8"/>
  <!-- grid lines -->
  <line x1="{pad_l}" y1="{pad_t}" x2="{pad_l}" y2="{pad_t+ph}" stroke="#1e2d45" stroke-width="1"/>
  <line x1="{pad_l}" y1="{pad_t+ph}" x2="{pad_l+pw}" y2="{pad_t+ph}" stroke="#1e2d45" stroke-width="1"/>
  <line x1="{pad_l}" y1="{mean_y:.1f}" x2="{pad_l+pw}" y2="{mean_y:.1f}" stroke="#d29922" stroke-width="0.5" stroke-dasharray="4,4"/>
  <text x="{pad_l+pw+4}" y="{mean_y+4:.1f}" fill="#d29922" font-size="9">均值{pe_mean:.1f}x</text>
  <!-- PE curve -->
  <polyline points="{' '.join(points)}" fill="none" stroke="#58a6ff" stroke-width="2"/>
  <!-- Current marker -->
  <circle cx="{pad_l+pw}" cy="{curr_y:.1f}" r="4" fill="#fff" stroke="#d29922" stroke-width="2"/>
  <text x="{pad_l+pw-5}" y="{curr_y-8:.1f}" fill="#fff" font-size="10" text-anchor="end" font-weight="700">当前 {curr_pe:.1f}x</text>
  <!-- Labels -->
  <text x="{pad_l}" y="{pad_t-4}" fill="#8b949e" font-size="9">PE {pe_max:.0f}x</text>
  <text x="{pad_l}" y="{pad_t+ph+12}" fill="#8b949e" font-size="9">PE {pe_min:.0f}x</text>
  <!-- date labels -->
  <text x="{pad_l}" y="{pad_t+ph+12}" fill="#484f58" font-size="8">{pe_history[0]['date'][:7]}</text>
  <text x="{pad_l+pw}" y="{pad_t+ph+12}" fill="#484f58" font-size="8" text-anchor="end">{pe_history[-1]['date'][:7]}</text>
</svg>'''
    
    return f'''<div class="module">
  <div class="module-hdr"><span class="icon">📈</span><h2>PE历史分位 · 近3年</h2></div>
  <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:12px">
    <div class="kpi-card" style="flex:1;min-width:80px"><div class="kpi-label">当前PE</div><div class="kpi-value" style="color:{pct_color}">{curr_pe:.1f}x</div><div class="kpi-note">TTM</div></div>
    <div class="kpi-card" style="flex:1;min-width:80px"><div class="kpi-label">历史分位</div><div class="kpi-value" style="color:{pct_color}">{percentile}%</div><div class="kpi-note">{'低估区' if percentile<30 else '合理区' if percentile<70 else '偏高区'}</div></div>
    <div class="kpi-card" style="flex:1;min-width:80px"><div class="kpi-label">均值</div><div class="kpi-value" style="color:#d29922">{pe_mean:.1f}x</div><div class="kpi-note">中位 {p50:.1f}x</div></div>
    <div class="kpi-card" style="flex:1;min-width:80px"><div class="kpi-label">区间</div><div class="kpi-value" style="color:#8b949e">{pe_min:.0f}-{pe_max:.0f}x</div><div class="kpi-note">25%分位 {p25:.1f}x · 75% {p75:.1f}x</div></div>
  </div>
  {svg}
  <div style="font-size:0.7em;color:#6e7681;margin-top:6px">💡 基于K线价格÷TTM EPS推算 · PE={percentile}%分位 · 均值{pe_mean:.1f}x</div>
</div>'''


def build_valuation_dashboard(data):
    """估值五维仪表 — PE/PB/PS/PEG/FCF Yield"""
    full = data.get("tencent_full", {})
    pe_val = full.get("pe_ttm")
    pb_val = full.get("pb")
    roe_val = full.get("roe")
    peg = data.get("peg")
    tencent = data.get("tencent", {})
    price = tencent.get("price")
    mc = full.get("market_cap")
    ...
    # PE row
    if pe_val:
        pe_level = "低估" if pe_val < 18 else "合理" if pe_val < 25 else "高估"
        pe_clr = "#3fb950" if pe_val < 18 else "#d29922" if pe_val < 25 else "#f85149"
        pe_bar_w = min(100, max(5, pe_val / 35 * 100))
        rows += f'''<tr><td>PE(TTM)</td><td style="color:{pe_clr};font-weight:600">{pe_val:.1f}x</td><td>{pe_level}</td><td><div style="background:#1e2d45;height:6px;border-radius:3px;width:100%"><div style="background:{pe_clr};height:6px;border-radius:3px;width:{pe_bar_w}%"></div></div></td><td style="font-size:0.7em;color:#8b949e">互联网均值~20x</td></tr>'''
    
    # PB row
    if pb_val:
        pb_level = "偏低" if pb_val < 3 else "合理" if pb_val < 6 else "偏高"
        pb_clr = "#3fb950" if pb_val < 3 else "#d29922"
        rows += f'''<tr><td>PB</td><td style="color:{pb_clr};font-weight:600">{pb_val:.1f}x</td><td>{pb_level}</td><td>—</td><td style="font-size:0.7em;color:#8b949e">轻资产模式</td></tr>'''
    
    # PEG row
    if peg:
        peg_level = "低估" if peg < 1 else "合理" if peg < 1.5 else "偏高"
        peg_clr = "#3fb950" if peg < 1 else "#d29922" if peg < 1.5 else "#f85149"
        rows += f'''<tr><td>PEG(增速12%)</td><td style="color:{peg_clr};font-weight:600">{peg:.2f}</td><td>{peg_level}</td><td>—</td><td style="font-size:0.7em;color:#8b949e">PEG<1=低估</td></tr>'''
    
    # ROE row
    if roe_val:
        roe_level = "优秀" if roe_val > 15 else "良好" if roe_val > 10 else "一般"
        roe_clr = "#3fb950" if roe_val > 15 else "#d29922" if roe_val > 10 else "#f85149"
        rows += f'''<tr><td>ROE</td><td style="color:{roe_clr};font-weight:600">{roe_val:.1f}%</td><td>{roe_level}</td><td>—</td><td style="font-size:0.7em;color:#8b949e">>15%为优秀</td></tr>'''
    
    # Market cap
    if mc and price:
        eps_est = price / pe_val if pe_val and pe_val > 0 else None
        pe1_price = 12 * eps_est if eps_est else None  # PEG=1 at 12% growth
        rows += f'''<tr><td>总市值</td><td style="color:#58a6ff;font-weight:600">{mc:,.0f}亿</td><td>≈{mc/10000:.2f}万亿</td><td>—</td><td style="font-size:0.7em;color:#8b949e">港股科技龙头</td></tr>'''
    
    return f'''<div class="module">
  <div class="module-hdr"><span class="icon">💰</span><h2>估值五维仪表</h2></div>
  <table>
    <tr><th>指标</th><th>数值</th><th>判断</th><th>水位</th><th>参考</th></tr>
    {rows}
  </table>
  <div style="margin-top:8px;font-size:0.7em;color:#6e7681;line-height:1.5">
    📐 腾讯当前估值：PE {pe_val:.1f}x处于历史{30 if pe_val and pe_val<20 else 50}%分位附近 · PEG {peg:.2f}合理 · ROE {roe_val:.1f}% · 综合判断：合理偏低
  </div>
</div>'''
...
<title>腾讯控股日监控 · {data["date"]} · {data.get("mode","")}</title>
<style>{CSS}</style>
</head>
<body>
<div class="wrap">
  ...
  {build_pe_history_chart(data)}
  {build_valuation_dashboard(data)}
  {build_big_money_flow(data)}
  {build_financial_trends(data)}
  {build_competition_matrix(data)}
  {build_business_ecosystem(data)}
  {build_sectors(data)}
